...
利用不同的方法 (k-NN, KDE) 和几种分布图，能够对 SDSS
的数据做出比较合理的密度估计，画出高密度区的范围.
...
\begin{Verbatim}[commandchars=\\\{\}]
\PY{c+c1}{\PYZsh{} 这里没有用 astroML.density\PYZus{}estimation 的 KNeighborsDensity 函数，因为我发现用这个会报错：}
...
\PY{c+c1}{\PYZsh{} 这个报错应该是 astroML 和 numpy 的版本不匹配问题，但是我是在本地环境完成作业的所以比较难修改.}
\PY{c+c1}{\PYZsh{} 因此我做了等价的替换，后面所有涉及到 astroML 的代码同理.}
...
\PY{c+c1}{\PYZsh{} cv\PYZhy{}Kfold 指的是，将数据分为 K 份，每次从中间取出 (K\PYZhy{}1) 份作为训练集，剩下 1 份作为测试集，}
\PY{c+c1}{\PYZsh{} 然后做 K 次取 MSE (Mean Squared Error, 均方误差) 的均值，作为最后这个模型的 MSE.}
\PY{c+c1}{\PYZsh{} cv\PYZhy{}loo 是这种方法的一个极限情况，每次只取一个数据点作为测试集.}
\PY{c+c1}{\PYZsh{} cv\PYZhy{}loo 更加精确，但是计算量也更大.}
...
\PY{k}{for} \PY{n}{method} \PY{o+ow}{in} \PY{n}{bw\PYZus{}methods}\PY{p}{:}
    ...
    \PY{k}{for} \PY{n}{n} \PY{o+ow}{in} \PY{n}{num\PYZus{}stars}\PY{p}{:}
        ...
        \PY{k}{elif} \PY{n}{method} \PY{o}{==} \PY{l+s+s1}{\PYZsq{}}\PY{l+s+s1}{cv\PYZhy{}Kfold}\PY{l+s+s1}{\PYZsq{}}\PY{p}{:}
            ...
            \PY{c+c1}{\PYZsh{} 一般而言，我们按照经验取 K = 5 或 10，}
            \PY{c+c1}{\PYZsh{} 取到 10 的时候和 cv\PYZhy{}loo 的差异就不大了，所以这里取 5 体现一下差异.}
...
\PY{c+c1}{\PYZsh{} 找了 2.1 问的结果，综合比较下来还是 cv\PYZhy{}loo 给出的 bins 比较合理.}
\PY{n}{best\PYZus{}bandwidth} \PY{o}{=} \PY{p}{[}\PY{l+m+mf}{17.25}\PY{p}{,} \PY{l+m+mf}{10.62}\PY{p}{]}
...
很明显，我们的结论和 bandwidth 以及是否抵消误差是强相关的，在实际做 KDE
的时候应该多采用不同的 bins 选择方案，找到最佳的 bins
数，同时合理考虑误差的分布情况，得到最终的结果.
